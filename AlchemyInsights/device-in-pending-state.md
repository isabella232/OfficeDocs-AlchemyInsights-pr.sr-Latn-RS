...
**Okruženje potvrde identiteta u oblaku (pomoću sinhronizacije sinhronizacije azure AD lozinke ili prolazne potvrde identiteta)**

Ovaj tok registracije je poznat i kao "Sinhronizuj spajanje".

Evo sloma onoga što se događa tokom procesa registracije:

1. Windows 10 otkrije zapis tačke povezivanja usluge (SCP) kada se korisnik prijavi na uređaj.
    ...
    **Naznaka:** Preporučujemo da omogućavate SCP u aktivnom direktorijumu i da koristite SCP na strani klijenta samo za početnu proveru valjanosti.

2. Windows 10 da komunicira sa uslugom Azure AD u okviru konteksta sistema da bi se potvrdio identitet u okviru usluge Azure AD.
...
3. Windows 10 generiše samo potpisani certifikat i skladišti ga ispod objekta na računaru u lokalnoj službi Active Directory. Ovo zahteva kontroluer domena na vidnom mestu.
...
6. Kada se sledeći korisnik prijavi Windows 10, registracija će biti dovršena.
    ...
    **Natpis:** Ako imate VPN, a logotip/prijavljivanje obustavljuje povezivanje domena, možete ručno da pokrenete registraciju. Da biste to uraditi:
    
    Lokalno `dsregcmd /join` na odzivu adminitracije ili daljinski putem usluge PSExec na računaru.\
    Na primer: `PsExec -s \\win10client01 cmd, dsregcmd /join`

Za uobičajene probleme sa Azure Active Directory registracijom uređaja, pogledajte [najčešća pitanja o uređajima.](https://docs.microsoft.com/azure/active-directory/devices/faq)
